...
fn shuffle_system_xanadu(iter: SingleComponentRefIterMut<'_, Position>) {
    for mut pos in iter {
        shuffle_system_refmut(&mut pos);
    }
}

fn increment_system_xanadu(iter: SingleComponentRefIterMut<'_, Position>) {
    for mut pos in iter {
        increment_system_refmut(&mut pos);
    }
}
